...
8. **Commands refer to roles; the EM resolves role → named agent.** Commands talk about "the Research Analyst", "the Business Analyst", etc. — never specific first names. The EM picks the concrete agent when spawning: in Team Mode, look up `engagement.json → team_members` and, when multiple instances of a role exist (parallel workstreams), pick the right one by workstream assignment + continuity rule (same role stays on the same workstream where possible). In Tool Mode, use the role default — Research Analyst → Sara, Business Analyst → Tom, Slide Architect → Lisa, QA Reviewer → James, Partner Advisor → Maria, Financial Modeler → Alex. Status updates and briefings to the Principal may use the resolved first name for readability; command files themselves stay role-based.
